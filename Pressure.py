    ...
    def calculate_pressure(self):
        ...
        with open(output_file, "a") as f:
            ...
            for cpt, array in enumerate(self.matrix):
                ...
                #f.write(f"{list}\n")
                list = [ele for ele in list if ele > cpt and  ele != self.indices[cpt]] ### Extremely tricky , double check ! Only works if number of segments is one !
                #print(list)
        ...
        with open(output_file, 'w') as f:
            ...
            for Ni in np.arange(np.sum(num_atoms)):
                ...
                #print("position_of_i:",position_of_i)
                ...
                #print("neighbor_positions:",neighbor_positions)
                #print("\n\n\n")
...
#                     
#                     #Check for Scenario 1: At least one particle is outside the cutoff radius (R + d)
#                     if (circle.check_position_inside_cutoff(A[1], A[2])) == "above" or  (circle.check_position_inside_cutoff(B[1], B[2])) == "above":
#                         New_Virial = 0
#                         total_virial += New_Virial
#                         continue  # Skip remaining checks and proceed to next iteration
#                     
#                     
#                     
...
for ts, ts2 in zip(universe.trajectory[:1], universe2.trajectory[:1]): 
    ...
    for segment in range(num_segments):
        ...
        cutoff=12
        # inner_radius+cutoff
        selected_atoms_inside = segment_ag[[np.linalg.norm(atom.position[1:3] - center_of_mass[1:3]) < inner_radius+cutoff  for atom in segment_ag]]
        ...
        #print(f"# of particles Inside cutoff: {len(selected_atoms_inside)} atoms")
        #from MDAnalysis.analysis import distances
